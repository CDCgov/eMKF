# R eMKF Macro Version 2.4: Rmkf Call Parameters

## Function Overview

The `Rmkf()` function is the main interface for the eMKF (extended Multilevel Kalman Filter) macro. Code in `emkf_macro_v24.R` must be sourced prior to every `Rmkf()` call, as the function removes custom samplers at the end of each call that must be recompiled.

**Last Updated:** MT. 30-Jan-2026
...
Rmkf(
  ...
  data = "",                                         # (character) Input dataset name should be in quotes as it is passed to Rmkf() by reference, as in SAS, instead of by value
  ...
  # Prior Parameters - AR(1) and Variance
  ...
  # Dirichlet Prior Options (Not Yet Implemented)
...
### Core Data Parameters
- **data**: Input dataset name (character)
- **outcome**: Column name for outcome variable
- **se**: Column name for standard error of outcome
- **neff**: Column name for effective sample size
- **by**: Stratification variable column name
- **group**: Grouping variable column name
- **time**: Timepoint variable column name

### Model Specification
- **breakPoint**: Timepoint where segment 2 starts
- **breakType**: Type of break ('full_break' or 'level_break')
- **bayesModel**: Model type (e.g., 'bma_cubic', 'indep_linear', etc.)
- **randomVars**: Use random (TRUE) or fixed (FALSE) sampling variances
- **ARmodel**: Autoregressive model type

### MCMC Settings
- **chains**: Number of MCMC chains (default: 4)
- **nbi**: Burn-in steps (default: 10,000)
- **nmc**: MCMC samples to retain (default: 50,000)
- **thin**: Thinning rate (default: 1)
- **seed**: Random seed for reproducibility

## Important Notes

- Source `emkf_macro_v24.R` before every `Rmkf()` call
- Custom samplers are removed after each call and must be recompiled
- Many prior parameters are determined empirically if left unspecified
- Dirichlet prior options are not yet implemented
