...
* Please refer to the SAS file [Compare-eMKF-to-MKF.sas](Testing-and-implementation/Compare-eMKF-to-MKF.sas) for sample code to implement the eMKF and replicate the functionality of the earlier SAS MKF macro.
...
* There should generally be at least *k*+4 timepoints to estimate a degree *k* polynomial trend. eMKF version 2.4 will check the model complexity against the number of available data points to avoid over-fitting.
  * For linear trends, 5 timepoints are usually needed.
  * For quadratic trends, 6 timepoints are usually needed.
  * For cubic trends, 7 timepoints are usually needed.
  * eMKF version 2.4 allows exceptions to this general *k*+4 rule-of-thumb based on the user-specified structure of the AR(1) random effects; see below.
...
### Time points
* eMKF allows for time points to be unequally spaced as well as fractional. As of version 2.4, to ensure a nonnegative autocorrelation coefficient for the AR(1) random effects, a truncated normal prior distribution is used. In version 1.4, negative draws were discarded (resulting in rejection of the corresponding Metropolis-Hastings random walk proposals) as they resulted in invalid AR(1) covariance matrices when time points were fractional, e.g., 2018.6.
...
* eMKF generally does not allow fitting a degree *k* polynomial trend (*k*=0,1,2,3) unless there are *k*+4 available time points. As of version 2.4, exceptions to this *k*+4 rule-of-thumb are allowed, depending on the user-specified structure of the AR(1) random effects; see below.
...
* eMKF uses (effective) sample size (NEFF) as degrees of freedom in the chi-squared distribution of group- and time-specific sampling variances, therefore NEFF must be supplied when survey data are used. 
...
* The SAS eMKF macro implements "independent" and "common" trend options, which were left out of the earlier SAS MKF macro, in addition to the "full" hierarchical Bayesian model (Setodji et al, 2011).
...
* The SAS eMKF macro replaces the call to the external .exe file in the earlier SAS MKF macro (which consisted of pre-compiled C code) with a call to SAS PROC MCMC.
* The SAS eMKF macro implements Gibbs sampling in SAS PROC MCMC by calling user defined samplers (UDS) that are custom-built and precompiled using SAS PROC FCMP.
* eMKF replaces the *z*-score-based convergence diagnostic used in the earlier SAS MKF macro with a robust version of the Gelman-Rubin diagnostic (Vehtari et al, 2021).
* eMKF applies the Gelman-Rubin diagnostic to *all* model parameters, not just for the true state predictions ('etas') as in the earlier SAS MKF macro.
...
### MLE-based estimation setting in the SAS eMKF macro
* In the SAS eMKF macro, any subset of the seven allowable models (indep_cubic, indep_quad, indep_linear; common_cubic, common_quad, common_linear; and dropped) can be averaged; see Appendix II Table in the eMKF Guidance Report (doi:10.15620/cdc/157496) (<https://www.cdc.gov/nchs/data/series/sr_02/sr02-209.pdf>). 
...
* The SAS eMKF macro initializes parameters to pass to SAS PROC NLMIXED using the appropriate 'by' group stratum/replication (SAS PROC REG). This differs from the earlier SAS MKF macro where only the first stratum/replication was used to initialize the regression coefficients across strata/replications.
* The SAS eMKF macro initializes parameters to pass to SAS PROC NLMIXED using the appropriate degree *k* polynomial regression (SAS PROC REG), including for *k*=0. This differs from the earlier SAS MKF macro where for *k*=0 (dropped), the intercept values were initialized at those from the linear regression *y*=*a*+*b*×*t* instead of *y*=*a*.
* For the dropped (*k*=0) case, the SAS eMKF macro only keeps the column vector of 1s in the *X* matrix (and subsequent matrix calculations for the MSE). This differs from the earlier SAS MKF macro where both the 1s and times (*t*s) column were kept in the *X* matrix.
...
* Polynomial trend combinations for segments 1 and 2 will be determined by the type of trend break specified (level-shift only vs. full trend break), the specified trend model for segment 1, and the number of available timepoints.
  * Models for segments 1 and 2 will be either both bma-xxx, both indep-xxx, both common-xxx, or both dropped (intercepts-only) models. In the SAS implementation, models for segments 1 and 2 can also be specified as "fully-Bayesian" models, as in the earlier SAS MKF macro. 
